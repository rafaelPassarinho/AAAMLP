...
class BERTDataset:
    def __init__(self, review, target):
        ...
        self.review = review
        self.target = target
        # we fetch max len and tokenizer from config_bert.py
    ...
    def __getitem__(self, item):
        ...
        # mask and token type ids which are needed for models
        # like BERT. here, review is a string
        ...
        # mask is 1 where we have input and 0 where we have
        # padding
        mask = inputs['attention_mask']
        # token type ids behave the same way as mask
        # in this especific case. in case of two sentences,
        # this is 0 for first sentence and 1 for second sentence
        token_type_ids = inputs['token_type_ids']

        # now we return everything
        # note that ids, mask and token_type_ids are all
        # are all long datatypes and target is float
        return {
